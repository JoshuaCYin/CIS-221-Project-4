...
public class LinkedCollection<T> implements CollectionInterface<T>  
{
  ...
  // public String toString() { // Prints list in given order
  //   String collectionString = ""; // Create a string to return
  //   LLNode<T> temp = head;
  //   while (temp != null) {
  //     collectionString = collectionString + temp.getInfo() + " "; // Concatenate every element in the collection to the string, separated by spaces
  //     temp = temp.getLink();
  //   }
  //   return collectionString; // Return the string
  // }

  public String toString() {  // Prints list in reverse of given order
    LLNode<T> temp = head;
    return recToString(temp);
  }

  private String recToString(LLNode<T> node) {
    if (node == null) {
      return "";
    }
    else {
      String collectionString = recToString(node.getLink());
      return collectionString + node.getInfo() + " ";
    }
  }
}